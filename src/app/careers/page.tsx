...
const benefits = [
  { icon: '💰', title: 'Competitive Salary', description: 'Market-rate compensation in EUR' },
  { icon: '🌴', title: 'Unlimited PTO', description: 'Take the time you need. We trust adults.' },
  { icon: '🏠', title: 'Remote First', description: 'Work from anywhere in the EU time zones' },
  { icon: '💻', title: 'Home Office Budget', description: '€2,000 setup budget + €50/month' },
  { icon: '📚', title: 'Learning Budget', description: '€1,000/year for courses and books' },
  { icon: '🚀', 'title': 'Equity Options', description: 'Own what you help build' },
];
    ...
    <div className="min-h-screen bg-bg">
      ...
      <section className="section">
        <div className="px-6 md:px-12 max-w-4xl">
          ...
          <h1 className="font-serif text-[clamp(42px,5vw,72px)] font-semibold leading-[1.02] tracking-tight text-fg mb-7 animate-fade-up delay-1">
            Build software<br />
            <em className="italic font-normal text-grey-600">that matters.</em>
          </h1>
        ...
        <div className="px-6 md:px-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {benefits.map((benefit, index) => (
              <div key={index} className="p-6 border border-grey-200">
                <div className="text-3xl mb-4">{benefit.icon}</div>
                <h3 className="text-[16px] font-bold text-fg mb-2">{benefit.title}</h3>
                <p className="text-[13px] text-grey-600">{benefit.description}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="section bg-grey-50">
        <div className="px-6 md:px-12">
          ...
          <div className="max-w-4xl space-y-4">
            {jobs.map((job, index) => (
              <div key={index} className="p-8 bg-white border border-grey-200 hover:border-fg transition-colors">
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-4">
                  <div className="flex-1">
                    <h3 className="text-[20px] font-bold text-fg mb-2">{job.title}</h3>
                    <p className="text-[14px] text-grey-600 mb-4">{job.description}</p>
                    <div className="flex flex-wrap gap-2">
                      {job.skills.map((skill) => (
                        <span key={skill} className="font-mono text-[10px] px-2 py-1 rounded bg-grey-50 border border-grey-200 text-grey-600">
                          {skill}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="flex flex-col gap-2 md:text-right">
                    ...
                    <div className="font-mono text-[11px] text-grey-400">{job.type}</div>
                  </div>
                </div>
                <div className="pt-4 border-t border-grey-100">
                  <Link
                    href={`/careers/${job.title.toLowerCase().replace(/\s+/g, '-')}`}
                    className="inline-flex items-center gap-2 text-[13px] font-semibold no-underline text-fg hover:text-grey-600 transition-colors"
                  >
                    View details →
                  </Link>
                </div>
          ...
          {/* No matching jobs */}
          <div className="max-w-4xl mt-12 p-8 bg-white border border-grey-200">
            <p className="text-[15px] text-grey-600 text-center">
      ...
      <section className="section bg-white">
        <div className="px-6 md:px-12 max-w-4xl">
          ...
          <h2 className="font-serif text-[clamp(36px,4vw,56px)] font-semibold leading-[1.08] tracking-tight text-fg mb-8">
            Our culture
          </h2>

          <div className="space-y-6">
            <div className="flex gap-4">
              <div className="w-8 h-8 rounded-full bg-fg text-bg flex items-center justify-center flex-shrink-0 font-mono text-sm font-bold">
                1
              </div>
              <div>
                <h3 className="text-[18px] font-bold text-fg mb-2">Ship first, meeting later</h3>
            ...
            <div className="flex gap-4">
              ...
              <div>
              <h3 className="text-[18px] font-bold text-fg mb-2">Own your work</h3>
              <p className="text-[14px] text-grey-600 leading-relaxed">
            ...
            <div className="flex gap-4">
              ...
              <div>
                <h3 className="text-[18px] font-bold text-fg mb-2">Profitable or gone</h3>
                <p className="text-[14px] text-grey-600 leading-relaxed">
            ...
            <div className="flex gap-4">
              ...
              <div>
                <h3 className="text-[18px] font-bold text-fg mb-2">Europe first</h3>
                <p className="text-[14px] text-grey-600 leading-relaxed">
